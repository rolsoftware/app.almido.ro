...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-3">
            @can('app:role-create')
                <a  class="btn btn-success btn-rounded btn-label waves-effect waves-light mb-2 me-2" href="{{ route('role.create') }}"  title="Adauga"><i class="bx bx-plus label-icon"></i> Adauga</a>
            @endcan
        </div>
        <div class="col-lg-6  text-center font-size-20">
            <h2 class="mb-4">Lista roluri</h2>
        </div>
                                        ...
                                        <td  class="text-center">
                                            <ul class="list-inline font-size-20 contact-links mb-0">
                                                ...
                                                @can('app:permission-edit')
                                                    <li class="list-inline-item"><a href="{{ route('role-permission.show',['role'=>$row]) }}" title="Permisiuni"><i class="text-info bx bx-shield-quarter"></i></a></li>
                                                @endcan

                                                @can('app:role-edit')
                                                    <li class="list-inline-item"><a href="{{ route('role.edit',$row) }}" title="Editeaza"><i class="text-success bx bx-edit-alt"></i></a></li>
                                                @endcan

                                                @can('app:role-delete')
                                                    <li class="list-inline-item">
                                                        <a href="" title="Sterge"><i class="text-danger delete-confirm bx bx-trash" data-id="{{ $row->id }}"></i></a>
                                                        <form id="delete-form-{{ $row->id }}" action="{{ route('role.destroy',$row) }}" method="POST" class="d-none">
                                                            @csrf
                                                            @method('delete')
                                                        </form>
                                                    </li>
